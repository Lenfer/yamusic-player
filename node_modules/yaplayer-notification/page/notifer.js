var ipc = require('ipc');
var page = {
	body: $('body'),
	cover: $('#coverId'),
	artist: $('#artistId'),
	title: $('#titleId')
};

var defaultCoverUrl = 'https://music.yandex.ru/blocks/common/default.80x80.png';

ipc.on('yaplayer:notification:show', function(track) {

	// Set track name
	page.title.html(track.title);

	// Set artist name
	var artist = _.map(track.artists, 'name').join(', ');
	page.artist.html(artist);

	// Set cover URL
	var coverUri = track.albums[0].coverUri;
	var url = coverUri ? [
		'https://',
		coverUri.replace('%%', '80x80')
	].join('') : defaultCoverUrl;
	page.cover.attr('src', url);

	fadeIn();
});


page.body.click(function() {
	console.log('Send yaplayer:notification:hide-immediately');
	ipc.send('yaplayer:notification:hide-immediately');
	fadeOut();
})


ipc.on('yaplayer:notification:hide', function() {
	fadeOut();
});


var etalon = page.body
	.css('backgroundColor')
	.replace(/[\d\.]+\)/, '');

var beginAlpha = 0.75;

function setAlpha(alpha) {
	page.body
		.css({
			opacity: alpha + 0.25,
			backgroundColor: etalon + alpha + ')'
		});
}

function fadeOut() {
	var alpha = beginAlpha;
	var id = setInterval(function() {
		alpha -= 0.1;
		if (alpha >= 0) {
			setAlpha(alpha);
		} else {
			clearInterval(id);
		}
	}, 10);
}

function fadeIn() {
	var alpha = 0;
	var id = setInterval(function() {
		alpha += 0.1;
		if (alpha >= 0.7) {
			setAlpha(alpha);
		} else {
			setAlpha(0.75);
			clearInterval(id);
		}
	}, 10);
}

fadeOut();
